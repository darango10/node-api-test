...
export class StocksController {
   ...
   */
  async listStocks(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { nextToken, limit } = req.query;

      logger.info({ nextToken, limit }, 'Processing list stocks request');

      const parsedLimit = limit ? Math.min(Math.max(parseInt(limit as string, 10), 1), 100) : 20;
      ...
      logger.error({ error: err.message }, 'List stocks request failed');
